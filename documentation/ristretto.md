# Why Ristretto? A Deep Dive into Our Policy Caching Strategy

> [!NOTE]
> **Clarification**: Ristretto is used strictly for caching **rate-limit policies** fetched from the PostgreSQL database (the Policy Engine) to prevent heavy database hits on every request. It is *not* used as the memory state store for the actual rate-limiting algorithms themselves (e.g., Token Bucket, Sliding Window), which manage their state independently.
...
### The Problem with Native Maps (Memory Bounds)
...
### The Ristretto Solution (Memory Bounds)
...
### The Problem with Native Maps (TTL)
